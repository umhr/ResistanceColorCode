package  
{
	...
	public class MoveManager 
	{
		...
		private function timer_timerComplete(event:TimerEvent):void 
		{
			...
			Tween24.tween(this, 0).onComplete(ngSound).delay(0.4).play();
			next();
		}
		public function ngSound():void {
			SoundManager.getInstance().play(SoundManager.NEXT);
		}
		...
		private function nextQuestion(oldFloor:int, newFloor:int):void 
		{
			...
			Tween24.parallel(
				...
				Tween24.tween(this, 0.5, null, { isTween:false } )
			).delay(0.5).onPlay(onPlay).onComplete(startTimer).play();
